...
\begin{itemize}
    \item każdego przejścia (krawędzi w~siatce) można użyć tylko raz,
...
\textbf{8.\ Jednokrotność użycia krawędzi i odwiedzin atrakcji:}
\begin{align}
& x_{ij} + x_{ji} \le 1 \quad \forall (i,j) \in E
   && \text{(krawędź nieskierowana użyta co najwyżej raz)} \label{eq:edge-unique}\\
& y_i \le 1 \quad \forall i \in A
   && \text{(atrakcja odwiedzona co najwyżej raz)} \label{eq:attr-unique}
\end{align}
...
podzbioru (klasy \emph{Orienteering Problem} z~ograniczeniami typowymi
i~zakazem powtórzeń krawędzi).
...
\begin{itemize}
    ...
    \item \textbf{Ścieżka jako konstrukcja pomocnicza.} W~implementacji
          ...
          a~także krawędzie wcześniej zużyte (twardy zakaz powtórzeń krawędzi,
          \eqref{eq:edge-unique}).
