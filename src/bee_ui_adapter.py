from bee_algorithm import generate_bee_population


def _bee_solution_key(sol: dict):
    return (
...
def _to_ui_path(bee_path):
    # Bee uses (row, col), UI uses (x, y)
    return [(c, r) for r, c in bee_path]


def _bee_evaluate_path(path, parsed_data):
...
def _ui_map_to_bee(map_data):
    ...
    for a in map_data.get("attractions", []):
        pos = (a["y"], a["x"])  # Bee uses (row, col), UI uses (x, y)
...
def solve_bee_ui(
    ...
    *,
    ...
    parsed_data = _ui_map_to_bee(map_data)
    ...
    def _on_iter(it, best, population, history):
        if on_iteration is None:
            return
        top = sorted(population, key=_bee_solution_key, reverse=True)[: max(1, int(top_k))]
        on_iteration(
            {
                "iteration": it,
                "history": history,
    ...
    result = generate_bee_population(
        parsed_data,
        ...
        dance_quality_threshold=dance_quality_threshold,
        abandon_patience=abandon_patience,
        seed=seed,
        deduplicate=False,
        evaluate_path_fn=_bee_evaluate_path,
