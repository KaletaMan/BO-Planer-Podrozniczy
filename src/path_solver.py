...
def evaluate_path(map_data: dict, path: list[tuple[int, int]]) -> dict:
    ...
    for cell in path[1:]:
        ...
        if not (0 <= x < w and 0 <= y < h):
            return {"cost": cost, "value_collected": value_collected,
        ...
        if cell not in visited_cells:
            ...
            if cell in attraction_map:
                value_collected += attraction_map[cell]
                attractions_visited.append(cell)

    feasible = cost <= budget
    reason = "OK" if feasible else f"Koszt {cost} > budżet {budget}."
    return {
        "cost": cost,
...
def solve_greedy_attractions(map_data: dict) -> list[tuple[int, int]]:
    ...
    while remaining:
        ...
        sub_path, sub_cost = _walk_toward(pos, target, weights, w, h, budget - cost)
